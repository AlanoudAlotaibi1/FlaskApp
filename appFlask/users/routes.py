...
from appFlask.models import User, Lesson
from flask import render_template , url_for,flash,redirect,request
from appFlask.users.forms import RegistrationForm,LoginForm,UpdateProfileForm,RequestResetForm,ResetPasswordForm
from appFlask import bcrypt,db
from flask_login import login_user , current_user,logout_user,login_required
from appFlask.helpers import save_picture
from appFlask.users.helpers import send_reset_email
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password,form.password.data):
            login_user(user, remember=form.remember.data) # it will create session
            next_page = request.args.get('next')
            flash('You have been logged in!', 'success')
            return redirect(next_page) if next_page else redirect(url_for("main.home"))
        else:
            flash('Login unSuccessfull, Please check credintial!', 'danger')
