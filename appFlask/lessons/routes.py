...
from appFlask.lessons.forms import NewLessonForm,LessonUpdateForm
from appFlask.courses.forms import NewCourseForm
from appFlask import db
from flask_modals import render_template_modal
from flask_login import current_user,login_required
from flask import abort
from appFlask.helpers import save_picture
from appFlask.lessons.helpers import get_previous_next_lesson ,delete_picture
...
def new_lesson():
    ...
    ## to know the request is from which form , i depend on the special coulmns in each form
    form =''
    ...
    modal = None if flag else 'newCourse' ## to know if we wll call the modal
...
@lessons.route("/<string:course>/<string:lesson_slug>") ## dynamic link
def lesson(lesson_slug,course):
    ...
    lesson = Lesson.query.get_or_404(lesson_id) ## it will check in db if didn't find the lesson it will return http request 404 'NOT found'
    return render_template('lesson_view.html',title=lesson.title,lesson=lesson, previous_lesson=previous_lesson,
